...
export default function UpdateBlog() {
  ...
  const [slugTouched, setSlugTouched] = useState(true); // existing posts already have a slug
  ...
  const handleSubmit = async (status) => {
    try {
      if (!formData.coverImage) {
        setError('You must have a cover image');
        return;
      }
      ...
      setSaving(status);
      setError(false);
      const res = await fetch(`/api/blog/update/${params.blogId}`, {
        method: 'POST',
  ...
  if (fetching) {
    return (
      <div className='min-h-screen bg-[#0E211B] flex items-center justify-center'>
        <p className='text-[#EFE9DD]/50'>Loading blog post...</p>
      </div>
    );
  }
